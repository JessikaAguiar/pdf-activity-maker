...
df_asp["Nome"] = df_asp["Nome"].astype(str)
df_controle["Aluno"] = df_controle["Aluno"].astype(str)

# Atualiza o status para 'ATENDIDOS' se a matrícula estiver na planilha de controle
df_asp.loc[(df_asp["Nome"].replace('—','')).isin(df_controle["Aluno"]), "Status"] = "Encontrado"
